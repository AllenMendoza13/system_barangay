...
class BMISClass
{
    ...
    public function create_blotter()
    {
        ...
        if (isset($_POST['create_blotter'])) {
            ...
            //$id_resident = $_POST['id_resident'];
            $last_case_no = $this->queryThis("SELECT case_no FROM tbl_blotter ORDER BY case_no DESC LIMIT 1");
            if ($last_case_no) {
                $last_number = intval(substr($last_case_no, 4));
                $new_case_no = $last_number + 1;
            } else {
                $new_case_no = 1;
            }
            //$last_case_no = 0;
            //$new_case_no = $last_case_no + 1;
            $case_no = date('Y') . str_pad($new_case_no, 4, '0', STR_PAD_LEFT);
            $lname = $_POST['lname'];
            $fname = $_POST['fname'];
            $mi = $_POST['mi'];
            $houseno = $_POST['houseno'];
            ...
            $case_respondent = $_POST['case_respondent'];

            $connection = $this->openConn();
            $stmt = $connection->prepare("INSERT INTO tbl_blotter (`case_no`, `lname`, `fname`, `mi`,
            `houseno`, `street`,`brgy`, `municipal`, `contact`, `narrative`, `case_name`, `case_respondent`)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

            $stmt->execute([$case_no, $lname, $fname, $mi, $houseno,  $street, $brgy, $municipal, $contact, $narrative, $case_name, $case_respondent]);
